...
while True:
    ...
    for title in titles:
        ...
        if title_check and imax_check:  # 박찬욱관
            # if title_check:                     #일반
            open_check = ("준비중" not in a)
            if open_check:
                bot.sendMessage(mc, "12/11 인터스텔라 아이맥스 오픈!")
                print("open")
            else:
                if not ready_printed:
                    print("준비중")
                    bot.sendMessage(mc, "12/11 인터스텔라 아이맥스 오픈준비중!!! 오픈알림 기다리지 말고 미리 들가서 날짜 계속 클릭해서 새로고침 추천")
                    ready_printed = True
